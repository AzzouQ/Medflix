...
//       .getToken()
//       .then((r) => {
//         database.ref(`/user/${user?.uid}/fcm`).update({ mobile: r.token });
//       })
...
//           // ...
//           database.ref(`/user/${user?.uid}/fcm`).update({ web: currentToken });
//         } else {
...
//   try {
//     const keys = await (await database.ref(`/user/${uid}/fcm`).once('value')).val();
